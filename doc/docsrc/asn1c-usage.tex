...
{\ttfamily -fcomplex-threshold=\emph{<N>}} & {\small Set the threshold for complex type detection. Types with \emph{N} or more optional complex members may become pointers when both \texttt{-findirect-choice} and \texttt{-fcompound-names} are enabled. This helps break circular dependencies. Default is 2. See \fref{sec:Circular-Dependencies} for details.}\\
...
contexts. When combined with \texttt{-findirect-choice}, enables enhanced circular dependency detection.}\\
{\ttfamily -findirect-choice} & {\small When generating code for a CHOICE type, compile the CHOICE
members as indirect pointers instead of declaring them inline. Also affects Information Object Class OPEN\_TYPE pointer generation and circular dependency resolution. Consider
using this option together with \texttt{-fno-include-deps}
to prevent circular references. See \fref{sec:Circular-Dependencies}.}\\
...
\subsubsection*{3. Combined \texttt{-findirect-choice} and \texttt{-fcompound-names}}

When both flags are enabled, the compiler uses an enhanced heuristic:

\begin{bash}
asn1c -findirect-choice -fcompound-names module.asn1
\end{bash}

Types with 2 or more optional complex members become pointers when:
\begin{itemize}
\item Both \texttt{-findirect-choice} and \texttt{-fcompound-names} are set
\item The type contains optional members that are constructed types (SEQUENCE, SET, CHOICE)
\item The threshold is met (default is 2, configurable via \texttt{-fcomplex-threshold})
\end{itemize}

\subsubsection*{4. The \texttt{-fcomplex-threshold} Parameter}

Control the threshold for complex type detection:

\begin{bash}
asn1c -findirect-choice -fcompound-names -fcomplex-threshold=3 module.asn1
\end{bash}

This sets the threshold to 3, meaning only types with 3 or more optional complex 
members will become pointers. The default value is 2.
...
\begin{bash}
asn1c -findirect-choice -fcompound-names F1AP-PDU-Descriptions.asn1 \textbackslash
      F1AP-IEs.asn1 F1AP-Containers.asn1 F1AP-Constants.asn1
\end{bash}

This combination:
\begin{enumerate}
\item Detects only real circular dependencies (not just type references)
\item Applies minimal pointer indirection needed to break cycles
\item Preserves direct embedding where safe, improving performance
\item Handles Information Object Class tables correctly
\end{enumerate}

\subsection*{Generated Code}

For a type that becomes a pointer due to circular dependency resolution:

\begin{example}
/* Original ASN.1:
 * Container ::= SEQUENCE {
 *     item1  ComplexType1 OPTIONAL,
 *     item2  ComplexType2 OPTIONAL,
 *     item3  SimpleType
 * }
 */

/* Generated C with -findirect-choice -fcompound-names: */
typedef struct Container {
    struct ComplexType1  *item1;  /* OPTIONAL, pointer due to heuristic */
    struct ComplexType2  *item2;  /* OPTIONAL, pointer due to heuristic */
    long                 item3;
} Container_t;
\end{example}

Without these flags, the compiler might fail with "circular dependency" errors, or 
generate code that causes C compiler errors.

\subsection*{Backward Compatibility}

The circular dependency detection improvements maintain backward compatibility:
\begin{itemize}
\item Default behavior unchanged: only actual cycles trigger pointer indirection
\item Flags must be explicitly enabled for enhanced behavior
